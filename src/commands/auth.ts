...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  thumbfast-cli auth set sk-abc123xyz")
  .action((token: string) => {
    setToken(token);
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  thumbfast-cli auth show\n  thumbfast-cli auth show --raw")
  .action((opts: { raw?: boolean }) => {
    if (!hasToken()) {
      log.warn("No token configured. Run: thumbfast-cli auth set <token>");
      return;
    }
